...
class SpiderManagerForScience(object):
    def __init__(self,ids_thread_num=4, project=None, content_process_num=2, content_thread_num=8,page_size=spiders.default_science_pagesize,  **kwargs):
        # 爬虫的状态信息
    ...
    def add_task(self, valid_urls):
        for url in valid_urls:
            if heplers.url_type(url) != 'sciencedirect':
                continue
            art_id = url.split('/')[-1]
            self.ids_queen.put({'id': art_id, 'retry_times': 0})
    ...
    def start_assist(self):
        ...
        for i in range(self.content_process_num):
            name = '%s SCIENCE_ASSIST_CONTEND_PROCESS-%02d' % (self.common_tag, int(i + 1))
            content_worker = _scienceContendWorker(kw_id=self.kw_id, name=name, project=self.project)
            content_worker.start()
            self.content_process.append(content_worker)
        self.contentP_status.value = 1
